...
class WallFollowingDemo(gym.Env):
    ...
    def is_constraint(self, x, y):
        ...
        if min_dist > 0.09 and min_dist < 0.11:
            return False
        return True
    ...
    def _generate_track(self):
        ...
        def quad_circle_generator(center_x, center_y, radius, smooth, right = True):
            res = []
            for i in range(smooth):
                angle2 = -0.25 * math.pi * (i-1) / smooth
                if not right:
                    angle2 +=0
                x2 = center_x + radius * math.sin(angle2)
                y2 = center_y - radius * math.cos(angle2)
        ...
        def qquad_circle_generator(center_x, center_y, radius, smooth, right = True):
            ...
            for i in range(smooth):
                ...
                res.append((x2,y2) )
            return res
        self._track_points += [(0.3, 0.25)]
        self._track_points += qquad_circle_generator(-0.1, 0.3, 0.3, 10, False)
        self._track_points += [(-0.3, 0.0)]
