...
class ConstMonitor(gym.Wrapper):
    """
    ...
    def step(self, action: Union[np.ndarray, int]) -> GymStepReturn:
        ...
        if done:
            ...
            ep_const = sum(self.constraints)
            ep_info = {"r": round(ep_rew, 6), "l": ep_len, "t": round(time.time() - self.t_start, 6), "c": ep_const}
            for key in self.info_keywords:
                ep_info[key] = info[key]
            self.episode_returns.append(ep_rew)
            self.episode_constraints.append(ep_const)
    ...
    def get_episode_constraints(self) -> List[float]:
        """
        Returns the rewards of all the episodes

        :return:
        """
        return self.episode_constraints
