...
class ConstVecMonitor(VecEnvWrapper):
    """
    ...
    def __init__(
        ...
        self.episode_constraints = None
        self.episode_old_constraints = None
    ...
    def reset(self) -> VecEnvObs:
        ...
        self.episode_constraints = np.zeros(self.num_envs, dtype=np.float32)
        self.episode_old_constraints = np.zeros(self.num_envs, dtype=np.float32)
    ...
    def step_wait(self) -> VecEnvStepReturn:
        ...
        def extract_old_constraint(info):
            return info.get("original_env_constraint", 0.0)

        constraints = np.array(list(map(extract_constraint, infos)))
        old_constraints = np.array(list(map(extract_old_constraint, infos)))
        assert rewards.shape == constraints.shape

        self.episode_constraints += constraints
        self.episode_old_constraints += old_constraints
        ...
        for i in range(len(dones)):
            if dones[i]:
                ...
                episode_constraint = self.episode_constraints[i]
                episode_old_constraint = self.episode_old_constraints[i]
                episode_info = {"r": episode_return, 
                                "l": episode_length, 
                                "t": round(time.time() - self.t_start, 6),
                                "wc": episode_constraint,
                                "c": episode_old_constraint,
                                }
                ...
                self.episode_constraints[i] = 0
                self.episode_old_constraints[i] = 0
